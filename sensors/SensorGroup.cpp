...
void SensorGroup::retranslate()
{
    ...
    m_sensorNames[u"power/.*/chargeRate"_s] = i18nc("Power group sensor", "[Group] Charging Rate");
...
QString SensorGroup::groupRegexForId(QString key /*Intentional copy*/)
{
    ...
    static const QRegularExpression powerExpr(u"power/.*/(.*)"_s);
    ...
    if (key.contains(cpuExpr)) {
        ...
        key.replace(powerExpr, u"power/.*/\\1"_s);
    } else {
        return QString{};
    }
