...
void ApplicationDataModel::cgroupPidsUpdated(CGroup *cgroup)
{
    auto itr = m_applications.find(applicationId(cgroup));
    if (itr == m_applications.end()) {
        return;
    ...
    itr.value().processes.insert(cgroup, CGroupDataModel::processesFor(cgroup));
}
...
void ApplicationDataModel::cgroupRemoved(CGroup *cgroup)
{
    ...
    application.processes.remove(cgroup);
...
{
    auto itr = m_applications.find(applicationId(cgroup));
    if (itr == m_applications.end() || itr.value().processes.isEmpty()) {
        return {};
    }

    QList<Process *> result;
    for (auto processList : std::as_const(itr.value().processes)) {
