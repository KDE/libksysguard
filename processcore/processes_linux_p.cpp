...
public:
    Private()
    {
        mBuffer.fill('\0', PROCESS_BUFFER_SIZE + 1);
...
void ProcessesLocal::Private::smapsThreadFunction(std::stop_token stopToken, ProcessesLocal *processes)
{
    while (!stopToken.stop_requested()) {
        ...
        for (auto pid : pids) {
            ...
            while (file.readLine(buffer.data(), buffer.size()) > 0) {
                ...
                if (parts.size() >= 2) {
                    if (parts.at(0) == u"Rss"_s) {
                        ...
                        fields.other += std::stoll(parts.at(1).toStdString());
            ...
            fields.lastUpdate = std::chrono::system_clock::now();
...
bool ProcessesLocal::Private::readProcStatm(const QString &dir, Process *process)
{
    ...
    fields.other = atol(parts.at(0)) * pageSize; // Other is used for vmSize in imprecise information.
...
bool ProcessesLocal::updateProcessInfo(long pid, Process *process)
{
    ...
    if (mUpdateFlags.testFlag(Processes::Smaps)) {
        ...
        if (std::chrono::system_clock::now() - process->memoryInfo()->precise.lastUpdate >= PreciseUpdateInterval) {
