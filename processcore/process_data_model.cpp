...
public:
    ...
    // be propagated.
    QList<QModelIndex> m_pendingChanges;
};
...
void ProcessDataModel::Private::beginRemoveRow(KSysGuard::Process *process)
{
    ...
    auto removed = std::ranges::remove_if(m_pendingChanges, [row](const QModelIndex &index) {
...
void ProcessDataModel::Private::handleChangedProcesses()
{
    ...
    // operations below.
    QList<QModelIndex> pendingChanges;
    ...
    Q_EMIT q->dataChanged(q->index(firstChangedRow, firstChangedColumn, parentIndex), q->index(lastChangedRow, lastChangedColumn, parentIndex));
}
